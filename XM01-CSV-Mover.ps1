...
Start-Transcript \\p-ucadm01.janus.cap\d$\Scripts\Logs\XM01-CSV-Mover-$datecode.LOG
...
# Generate a move request for each element of the array
    $XM1_1 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
...
# Generate a move request for each element of the array
    $XM1_2 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
# IF the move failed, we need to alert the on-call person
    if ($movesuccess -ne $true)
        {
...
# Generate a move request for each element of the array
    $XM1_3 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
# IF the move failed, we need to alert the on-call person
    if ($movesuccess -ne $true)
        {
...
# Generate a move request for each element of the array
    $XM1_4 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
...
# Generate a move request for each element of the array
    $XM1_5 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
...
# Generate a move request for each element of the array
    $XM1_6 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
...
# Generate a move request for each element of the array
    $XM1_7 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
...
# Generate a move request for each element of the array
    $XM1_8 | New-MoveRequest -targetdatabase "$store" -AcceptLargeDataLoss -baditemlimit Unlimited -DomainController "$(Get-JDCs -GC)"  -MRSServer $((Get-TransportServer | Where {$_.Name -like "*P-UCUS*"} | Get-Random).Name) -confirm:$false
    $movesuccess=$?
