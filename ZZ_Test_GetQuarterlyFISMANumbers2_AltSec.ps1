...
$QSDate = '01/01/2017 00:00:01 AM'
$QEDate = '03/31/2017 11:59:59 PM'
...
<#Removing SRV
$a = get-aduser -filter {Enabled -eq $true} -server isvdendc01.srv.doi.net -properties description,distinguishedName,SmartcardLogonRequired,altSecurityIdentities,displayname,extensionAttribute5,sn,GivenName,mail,UserPrincipalName,CanonicalName,whenCreated
$b = $a | where { $_.Enabled -eq $True }
$c = $b | where {$_.distinguishedName -notlike "*Messaging*"}
$s = $c | where {($_.distinguishedName -like "*service*") -or ($_.displayname -like "*Service*") -or ($_.description -like "*Service*") -or ($_.samaccountname -like "svc*")}
$d1 = $c | where {($_.DistinguishedName -notlike "*service*")}
...
$1Users = $i | where {$_.extensionAttribute5 -like "*-*"}
$NewUsers = $1Users | where {$_.whenCreated -gt $QSDate -and $_.whenCreated -lt $QEDate}
$2DA = foreach ($z in (get-adgroup 'Domain Admins' -server isvdendc01.srv.doi.net -properties member).member){get-aduser -server isvdendc01.srv.doi.net $z -properties description,distinguishedName,SmartcardLogonRequired,altSecurityIdentities,displayname,extensionAttribute5 }
$1DA = $2DA | where {$_.Enabled -eq "True"}
$1SA = $i | where {$_.samaccountname -like "*-sa"}
$1WA = $i | where {$_.samaccountname -like "*-wa"}
$1DAP = $1DA | Where { $_.SmartcardLogonRequired -eq $True }
...
$NU = $NewUsers | measure
$NU = $NU.count
$1DA | select surname,givenname,displayname,samaccountname,Enabled,Description,distinguishedName,SmartcardLogonRequired,@{name="altSecurityIdentities";expression={$_.altSecurityIdentities -join ";"}}  | export-csv C:\Temp\FismaQuarterlyOutput\SRVDA.csv -Delimiter ";" -nti
$1SA | select surname,givenname,displayname,samaccountname,Enabled,Description,distinguishedName,SmartcardLogonRequired,@{name="altSecurityIdentities";expression={$_.altSecurityIdentities -join ";"}}  | export-csv C:\Temp\FismaQuarterlyOutput\SRVSA.csv -Delimiter ";" -nti
$1WA | select surname,givenname,displayname,samaccountname,Enabled,Description,distinguishedName,SmartcardLogonRequired,@{name="altSecurityIdentities";expression={$_.altSecurityIdentities -join ";"}}  | export-csv C:\Temp\FismaQuarterlyOutput\SRVWA.csv -Delimiter ";" -nti
$s | select surname,givenname,displayname,samaccountname,Enabled,Description,distinguishedName,SmartcardLogonRequired,@{name="altSecurityIdentities";expression={$_.altSecurityIdentities -join ";"}}  | export-csv C:\Temp\FismaQuarterlyOutput\SRVService.csv -Delimiter ";" -nti
$NewUsers | select surname,givenname,mail,whenCreated | export-csv C:\Temp\FismaQuarterlyOutput\SRVNewUsers.csv -Delimiter ";" -nti
write-output "SRV,$Users,0,0,$DA,$DAP,$BA,$BAP,$SA,$SAP,$WA,$WAP,$SER,$NU" | out-file c:\temp\fismaquarterly1.txt -append
#>
...
#$a = get-aduser -filter {Enabled -eq $true} -properties description,distinguishedName,SmartcardLogonRequired,altSecurityIdentities,sn,GivenName,mail,UserPrincipalName,CanonicalName,whenCreated -SearchBase "OU=DOI_BC,DC=DOI,DC=NET"
#$b = $a | where { $_.Enabled -eq $True }
...
#$a = get-aduser -filter {Enabled -eq $true} -properties description,distinguishedName,SmartcardLogonRequired,altSecurityIdentities,sn,GivenName,mail,UserPrincipalName,CanonicalName,whenCreated -SearchBase "OU=DOI_OS,DC=DOI,DC=NET"
#$b = $a | where { $_.Enabled -eq $True }
...
#$a = get-aduser -filter {Enabled -eq $true} -properties description,distinguishedName,SmartcardLogonRequired,altSecurityIdentities,sn,GivenName,mail,UserPrincipalName,CanonicalName,whenCreated -SearchBase "OU=DOI_OHA,DC=DOI,DC=NET"
#$b = $a | where { $_.Enabled -eq $True }
...
$All2 | select surname,givenname,displayname,samaccountname,Enabled,Description,distinguishedName,SmartcardLogonRequired | Export-CSV 'C:\Temp\FismaQuarterlyOutput\FISMA 2.5_2.5.1.csv' -nti
...
#Domain,Users,EnterpriseAdmins,EnterpriseAdmins_PIV_Enforced,DomainAdmins,DomainAdmins_PIV_Enforced,BureauAdmins,BureauAdmins_PIV_Enforced,ServerAdmins,ServerAdmins_PIV_Enforced,WorkstationAdmins,WorkstationAdmins_PIV_Enforced,Service_Accounts,New_UserAccounts
