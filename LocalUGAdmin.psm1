...
{ 
 <# 
   ...
   .Description 
    This function changes a local user password and unlocks it
   .Example 
 ...
 $lockedout = 16 # ADS_UF_LOCKOUT enumeration value from SDK 
 $user = [ADSI]"WinNT://$computerName/$username,user" 
 $user.setpassword($password)  
 $user.SetInfo() 
 if ($user.UserFlags[0] -BAND $lockedout)
    {
        $user.invokeSet("userFlags", ($user.userFlags[0] -BXOR $lockedout))
        $user.commitChanges()
    }
